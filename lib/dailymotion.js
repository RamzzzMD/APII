...
async function getVideoInfo(url) {
    ...
    // Amankan cookies saat request metadata
    const metadataRes = await client.get(playerUrl, { headers: COMMON_HEADERS });
    const metadata = metadataRes.data;

    // Prioritaskan kualitas tinggi jika tersedia, atau auto
    const streamUrl = metadata.qualities?.['1080']?.[0]?.url || 
                     metadata.qualities?.['720']?.[0]?.url ||
                     metadata.qualities?.['480']?.[0]?.url ||
                     metadata.qualities?.['auto']?.[0]?.url;

    if (!streamUrl) throw new Error("Gagal menemukan link stream video.");

    // Ambil cookies terbaru dari jar untuk URL player tersebut
    const cookies = await jar.getSetCookieStrings(playerUrl);
